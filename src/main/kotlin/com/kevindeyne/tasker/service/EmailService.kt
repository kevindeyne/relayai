...
@Component
open class EmailService(var userRepository: UserRepository, var projectRepository: ProjectRepository, var passwordEncoder : PasswordEncoder, val mailSender : JavaMailSender, val environment: Environment) {
	...
	fun sendActivationMail(key : String, email : String) {
		if(!environment.getActiveProfiles().contains("dev")){
			...
			helper.setText("<!doctype html>" +
					...
					"                        <p style=\"font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;\">Welcome to RelayAI. We just need to activate your account before we can continue. The button below will bring you to a time-sensitive, unique URL.</p>" +
					...
					"                                  <tbody>" +
					"                                    <tr>" +
					"                                      <td style=\"font-family: sans-serif; font-size: 14px; vertical-align: top; background-color: #3498db; border-radius: 5px; text-align: center;\"> <a href=\"https://www.relayai.io/activation/$key\" target=\"_blank\" style=\"display: inline-block; color: #ffffff; background-color: #3498db; border: solid 1px #3498db; border-radius: 5px; box-sizing: border-box; cursor: pointer; text-decoration: none; font-size: 14px; font-weight: bold; margin: 0; padding: 12px 25px; text-transform: capitalize; border-color: #3498db;\">Activate my account</a> </td>" +
					"                                    </tr>" +
					"                                  </tbody>" +
					"                                </table>" +
					"                              </td>" +
